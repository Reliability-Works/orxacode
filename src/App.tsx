...
export default function App() {
  ...
  const [archivedBackgroundAgentIds, setArchivedBackgroundAgentIds] = useState<Record<string, string[]>>({});
  const [hiddenBackgroundSessionIdsByProject, setHiddenBackgroundSessionIdsByProject] = usePersistedState<Record<string, string[]>>(
    "orxa:hiddenBackgroundSessionIdsByProject:v1",
    {},
  );
  ...
  const composerPlaceholder = activePresentationProvider === "opencode"
  ...
  useEffect(() => {
    ...
    const unsubscribe = events.subscribe((event) => {
      ...
      if (event.type === "opencode.project") {
        ...
        applyOpencodeStreamEvent(event.payload.directory, event.payload.event);
        ...
        if (kind === "session.error") {
          ...
          if (!useInterruptedReason) {
            setStatusLine(detail);
          }
          if (useRecoverableReason && !useInterruptedReason) {
  ...
  const selectedBackgroundAgentSessionID =
    selectedBackgroundAgent?.provider === "opencode" ? (selectedBackgroundAgent.sessionID ?? null) : null;
  const selectedBackgroundAgentPrompt =
    selectedBackgroundAgent?.provider === "opencode" ? (selectedBackgroundAgent.prompt ?? null) : null;
  ...
  const backgroundAgentDetail = useMemo<ReactNode>(() => {
    ...
    const projected = activeProjectDir && selectedBackgroundAgentSessionID
      ? selectSessionPresentation({
          provider: "opencode",
          directory: activeProjectDir,
    ...
    const normalizedPrompt = selectedBackgroundAgentPrompt ? normalizeTranscriptText(selectedBackgroundAgentPrompt) : null;
    ...
    const filteredRows = projected.rows.filter((row) => {
      ...
      const rowText = normalizeTranscriptText(
        row.sections
          .filter((section): section is Extract<(typeof row.sections)[number], { type: "text" }> => section.type === "text")
  ...
  }, [activeProjectDir, openReferencedFile, selectedBackgroundAgent, selectedBackgroundAgentPrompt, selectedBackgroundAgentSessionID]);
  const backgroundAgentTaskText = useMemo(() => {
    ...
    return normalizedPrompt && normalizeTranscriptText(taskText) === normalizedPrompt ? null : taskText;
  }, [activeProjectDir, selectedBackgroundAgent, selectedBackgroundAgentPrompt, selectedBackgroundAgentSessionID]);
