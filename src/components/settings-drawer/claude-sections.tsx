    ...
    <section className="settings-section-card settings-pad settings-server-grid">
      ...
      <div className="settings-codex-field-row">
        <button
          ...
          onClick={() => {
            void window.orxa.app
              .writeTextFile('~/.claude/settings.json', claudeSettingsJson)
        ...
        <button
          ...
          onClick={() => {
            void window.orxa.app
              .readTextFile('~/.claude/settings.json')
              .then(content => {
                setClaudeSettingsJson(content)
                setFeedback('settings.json refreshed')
              })
              .catch((error: unknown) =>
                setFeedback(error instanceof Error ? error.message : String(error))
              )
    ...
    <section className="settings-section-card settings-pad settings-server-grid">
      ...
      <div className="settings-codex-field-row">
        <button
          ...
          onClick={() => {
            void window.orxa.app
              .writeTextFile('~/.claude/CLAUDE.md', claudeMd)
        ...
        <button
          ...
          onClick={() => {
            void window.orxa.app
              ...
              .then(content => {
                setClaudeMd(content)
                setFeedback('CLAUDE.md refreshed')
              })
              .catch((error: unknown) =>
                setFeedback(error instanceof Error ? error.message : String(error))
              )
    ...
    <section className="settings-section-card settings-pad settings-server-grid">
      ...
      <div className="settings-server-status-card">
        <div className="settings-server-status-row">
          ...
          <span className="settings-server-status-value">
            {appPreferences.permissionMode ?? 'ask-write'}
          </span>
        </div>
      </div>
      ...
      <select
        ...
        onChange={e =>
          onAppPreferencesChange({
            ...appPreferences,
            permissionMode: e.target.value as 'ask-write' | 'yolo-write',
          })
        }
      >
