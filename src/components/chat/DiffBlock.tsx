  ...
  return diff.split('\n').flatMap((line, index) => {
    ...
    return (
      <div key={`${index}:${oldLabel}:${newLabel}:${line}`} className={className}>
        <span className="diff-block-line-number diff-block-line-number--old" aria-hidden="true">
          {oldLabel}
        </span>
        <span className="diff-block-line-number diff-block-line-number--new" aria-hidden="true">
          {newLabel}
        </span>
...
function PathLink({
  ...
  return (
    ...
    >
      <span className="message-file-link-name">{label}</span>
    </span>
  )
}
...
function DiffBlockHeader({
  ...
  return (
    ...
    >
      ...
      {onOpenPath ? (
        <PathLink reference={safePath} label={displayName} onOpenPath={onOpenPath} />
      ) : (
        <span className="diff-block-path">{displayName}</span>
      )}
      {stats.insertions !== undefined ? (
        <span className="diff-block-stat diff-block-stat--add">+{stats.insertions}</span>
      ) : null}
      {stats.deletions !== undefined ? (
        <span className="diff-block-stat diff-block-stat--del">-{stats.deletions}</span>
      ) : null}
      {hasContent ? (
...
export const DiffBlock = memo(function DiffBlock({
  ...
  const stats = useMemo(
    () => deriveDiffStats(diff, insertions, deletions),
    [deletions, diff, insertions]
  )
  const renderedDiffLines = useMemo(() => (diff ? renderDiffLines(diff) : null), [diff])

  return (
    <div
      className={`diff-block${expanded && hasContent ? ' diff-block--expanded' : ''}${hasContent ? ' diff-block--collapsible' : ''}`}
    >
      <DiffBlockHeader
        expanded={expanded}
