...
function EntryStatusDot({ status }: { status: ExploreEntry['status'] }) {
  return (
    <span className={`explore-entry-status explore-entry-status--${status}`} aria-label={status} />
  )
}
  ...
  return (
    <div className="explore-group">
      ...
      >
        ...
        <span
          className={`explore-group-label${isExploring ? ' explore-group-label--exploring' : ''}`}
        >
          {label}
        </span>
        ...
        <div className="explore-group-entries">
          {item.entries.map(entry => (
            <div key={entry.id} className="explore-entry">
              ...
              {entry.detail ? <span className="explore-entry-detail">{entry.detail}</span> : null}
