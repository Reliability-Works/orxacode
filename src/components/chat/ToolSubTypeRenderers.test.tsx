...
describe('BashTool', () => {
  ...
  it('strips ANSI codes from output', () => {
    render(
      <BashTool command="echo hi" output="\x1b[32mhello\x1b[0m" status="completed" exitCode={0} />
    )
  ...
  it('shows exit code in CommandOutput', () => {
    render(<BashTool command="false" output="err" exitCode={1} status="completed" />)
    fireEvent.click(screen.getByRole('button'))
...
describe('EditTool', () => {
  ...
  it('renders +/- stats in subtitle', () => {
    render(<EditTool path="src/index.ts" status="completed" insertions={5} deletions={2} />)
  ...
  it('renders DiffBlock when diff is provided', () => {
    ...
    render(<EditTool path="src/b.ts" status="completed" diff={diff} insertions={1} deletions={1} />)
