...
describe('ToolCallCard', () => {
  ...
  it('expands on click when output is provided', () => {
    render(<ToolCallCard title="run_command" status="completed" output="hello world" />)
  ...
  it('keeps the same title visible when expanded', () => {
    render(
      <ToolCallCard
        title="Ran npm install convex"
        expandedTitle="Ran command"
        status="completed"
        output="done"
      />
    )
...
describe('ThinkingRow', () => {
  ...
  it('hides expandable thinking summaries until the disclosure is opened', () => {
    render(
      <ThinkingRow
        summary="Planning the next edits"
        content="Create package.json and install dependencies."
      />
    )
...
describe('ToolGroup', () => {
  ...
  it('is collapsed by default', () => {
    render(<ToolGroup items={[<span key="a">visible content</span>]} count={1} defaultCollapsed />)
    ...
    render(
      <ExploreRow
        item={{
          ...
          status: 'exploring',
          entries: [
            { id: 'entry-1', kind: 'search', label: 'Searched for foo', status: 'running' },
          ],
        }}
      />
    )
