...
function usePlanCardBubbleState(content: string) {
...
function PlanCardBubbleActions({
  copied,
  onCopy,
  onDownload,
}: {
...
function PlanCardBubbleBody({
  ...
  content: string
  hasRenderableMarkdown: boolean
  onOpenFileReference?: (reference: string) => void
}) {
  return (
    <div ref={bodyRef} className="plan-card-bubble-body">
      {hasRenderableMarkdown ? (
        <TextPart content={content} role="assistant" showCopy={false} onOpenFileReference={onOpenFileReference} />
      ) : (
...
function PlanCardBubbleToggle({
  ...
  return (
    <button type="button" className="plan-card-bubble-toggle" onClick={onToggle} aria-expanded={expanded}>
      {expanded ? 'Collapse plan' : 'Expand plan'}
      <svg className={`plan-card-bubble-chevron${expanded ? ' plan-card-bubble-chevron--up' : ''}`} width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
...
function PlanCardBubble({
  ...
  const {
    ...
    isOverflowing,
    setExpanded,
  } = usePlanCardBubbleState(content)
  ...
  return (
    ...
    >
      <div className="plan-card-bubble-header">
        ...
        <PlanCardBubbleActions copied={copied} onCopy={handleCopy} onDownload={handleDownload} />
      </div>
      <PlanCardBubbleBody bodyRef={bodyRef} content={content} hasRenderableMarkdown={hasRenderableMarkdown} onOpenFileReference={onOpenFileReference} />
      <PlanCardBubbleToggle expanded={expanded} isOverflowing={isOverflowing} onToggle={() => setExpanded(current => !current)} />
    </article>
  )
}
...
export function UnifiedTimelineRowView({
  ...
  switch (row.kind) {
    case 'message':
      return renderMessageRow(row, onOpenFileReference)
    case 'thinking':
      return <ThinkingRow summary={row.summary} content={row.content} />
    case 'tool':
      return renderToolRow(row)
    case 'diff':
      return renderDiffRow(row, onOpenFileReference)
    case 'diff-group':
      return renderDiffGroupRow(row, onOpenFileReference)
    case 'tool-group':
      return renderToolGroupRow(row, onOpenFileReference)
    case 'context':
      return (
    ...
    case 'compaction':
      return renderCompactionRow()
    case 'turn-divider':
      return renderTurnDividerRow(row)
    case 'plan-card':
      return (
        <article className="message-card message-assistant">
    ...
    default:
      return renderNullRow()
  }
}

function renderNullRow() {
  return null
}
function renderMessageRow(
  row: Extract<UnifiedTimelineRenderRow, { kind: 'message' }>,
  onOpenFileReference?: (reference: string) => void
) {
  return (
    <MessageTurn>
...
function renderToolRow(row: Extract<UnifiedTimelineRenderRow, { kind: 'tool' }>) {
...
function renderDiffRow(
  row: Extract<UnifiedTimelineRenderRow, { kind: 'diff' }>,
  onOpenFileReference?: (reference: string) => void
) {
  return (
    <article className="message-card message-assistant">
...
function renderDiffGroupRow(
  row: Extract<UnifiedTimelineRenderRow, { kind: 'diff-group' }>,
  onOpenFileReference?: (reference: string) => void
) {
  return (
    <article className="message-card message-assistant">
...
function renderToolGroupRow(
  row: Extract<UnifiedTimelineRenderRow, { kind: 'tool-group' }>,
  onOpenFileReference?: (reference: string) => void
) {
  ...
  return (
    <article className="message-card message-assistant">
      <ToolGroup label={row.title} count={toolGroupItems.length} items={toolGroupItems} />
    </article>
  )
}

function renderNoticeRow(row: Extract<UnifiedTimelineRenderRow, { kind: 'notice' }>) {
  return (
    <article
    ...
    >
      <header className="message-header">
        ...
        {row.timestamp ? (
          <span className="message-time">{new Date(row.timestamp).toLocaleTimeString()}</span>
        ) : null}
      </header>
...
function renderTurnDividerRow(row: Extract<UnifiedTimelineRenderRow, { kind: 'turn-divider' }>) {
  const timeLabel = row.timestamp
    ? new Date(row.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
    : undefined
  const durationLabel =
    row.durationSeconds !== undefined && row.durationSeconds > 0
