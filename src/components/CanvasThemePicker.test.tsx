...
function renderPicker(themeOverrides: Partial<CanvasTheme> = {}, handlers: { onThemeChange?: (theme: Partial<CanvasTheme>) => void; onClose?: () => void } = {}) {
  const onThemeChange = handlers.onThemeChange ?? (vi.fn() as unknown as (theme: Partial<CanvasTheme>) => void);
...
describe("CanvasThemePicker", () => {
  it("renders all 6 preset swatches", () => {
    ...
    for (const preset of CANVAS_PRESETS) {
      expect(screen.getByRole("button", { name: `Apply ${preset.label} theme` })).toBeInTheDocument();
    }
  });
