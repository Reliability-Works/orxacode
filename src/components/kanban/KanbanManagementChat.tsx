...
import type { KanbanManagementPromptResult, KanbanManagementSession, KanbanProvider } from "@shared/ipc";
...
export function KanbanManagementChat({ workspaceDir }: Props) {
  ...
  const initSession = useCallback(async (p: KanbanProvider) => {
    setError(null);
    try {
  ...
  useEffect(() => { void initSession(provider); }, [initSession, provider]);
  ...
  return (
    <section className="kanban-management">
      <div className="kanban-management-header">
        <div className="kanban-segmented-control">
          {(["opencode", "codex", "claude"] as const).map((p) => (
            <button key={p} type="button" className={provider === p ? "active" : ""} onClick={() => { setProvider(p); setLastResult(null); }}>
              {providerLabel(p)}
            </button>
          ))}
        </div>
      ...
      {error ? <p className="skills-error" style={{ padding: "0 0 8px" }}>{error}</p> : null}

      <div className="kanban-management-transcript" ref={scrollRef}>
        {transcript.map((item) => (
          <div key={item.id} className={`kanban-management-message kanban-management-message--${item.role}`}>
            <div className="kanban-management-message-icon">
              {item.role === "user" ? <User size={12} /> : item.role === "assistant" ? <Bot size={12} /> : null}
            </div>
        ...
        {transcript.length === 0 ? (
          <div className="kanban-empty-state">
            Send a prompt to manage your board. Try &quot;Break down this feature into tasks&quot; or &quot;Create a task to fix the login bug&quot;.
          </div>
        ) : null}
      ...
      <div className="kanban-management-input">
        ...
        />
        <button type="button" className="kanban-primary-btn" disabled={sending || !prompt.trim()} onClick={() => void handleSend()}>
          <Send size={13} />
