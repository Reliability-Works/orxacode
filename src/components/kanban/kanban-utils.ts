...
export function scheduleSummary(schedule: { type: string; time?: string; days?: number[]; intervalMinutes?: number }) {
  if (schedule.type === "interval") {
    return `Every ${schedule.intervalMinutes} min`;
  }
  const labels = ["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"];
  const days = (schedule.days?.length ?? 0) === 7 ? "Daily" : (schedule.days ?? []).map((day) => labels[day]).join(" ");
