...
export function KanbanTaskTerminal({ workspaceDir, taskId, open, onClose }: Props) {
  ...
  const connectionRef = useRef<{ ptyID: string; directory: string } | null>(null);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const managed = createManagedTerminal(containerRef.current, {
          fontSize: 12,
          lineHeight: 1.3,
        ...
        const inputDisposable = managed.terminal.onData((data) => {
          if (connectionRef.current) {
            void window.orxa.terminal.write(connectionRef.current.directory, connectionRef.current.ptyID, data);
          }
        });
        ...
        const unsubscribe = window.orxa.events.subscribe((event) => {
          if (event.type === "pty.output") {
            ...
            if (payload.ptyID === connectionRef.current?.ptyID && payload.directory === connectionRef.current?.directory) {
        ...
        const observer = new ResizeObserver(() => {
          ...
          if (dims && connectionRef.current) {
            void window.orxa.terminal.resize(connectionRef.current.directory, connectionRef.current.ptyID, dims.cols, dims.rows);
          }
        });
  ...
  return (
    <div className="kanban-task-terminal">
      <div className="kanban-task-terminal-header">
        <span>Terminal {status === "connecting" ? "(connecting…)" : status === "error" ? "(disconnected)" : ""}</span>
        <button type="button" className="kanban-icon-btn" onClick={handleClose} title="Close terminal">
          <X size={12} />
        </button>
      </div>
