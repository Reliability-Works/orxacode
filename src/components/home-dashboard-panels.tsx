...
type DashboardTabsProps = {
  activeTab: 'opencode' | 'codex' | 'claude'
  onTabChange: (tab: 'opencode' | 'codex' | 'claude') => void
}
...
type DashboardLatestSessionsSectionProps = {
  loading: boolean
...
export function DashboardLatestSessionsSection({
  loading,
  recentSessions,
  onRefresh,
  onAddWorkspace,
  onOpenSettings,
}: DashboardLatestSessionsSectionProps) {
  return (
    <section className="dashboard-section">
      <div className="dashboard-section-title">
      ...
      <div className="dashboard-session-grid">
        ...
        {recentSessions.length === 0 ? (
          <p className="dashboard-empty">No sessions yet. Create one from a workspace.</p>
        ) : null}
      </div>
...
export function DashboardUsagePanel({
  ...
  loading,
  onRefresh,
}: DashboardUsagePanelProps) {
  const emptyMessage = loading
    ? `Loading ${providerName} usage data...`
    : `No ${providerName} usage data yet. Click refresh to load.`
      ...
      <section className="dashboard-section">
        <div className="dashboard-section-title">
          ...
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px' }}>
            <small>{usage?.updatedAt ? `updated ${timeAgo(usage.updatedAt)}` : ''}</small>
            <button type="button" onClick={onRefresh} disabled={loading}>
        ...
        {usage ? (
          <div className="dashboard-metric-grid">
            <article className="dashboard-metric">
              <strong className="dashboard-metric-value">{usage.totalThreads || sessionCount}</strong>
              <span className="dashboard-metric-label">
                {providerName === 'Codex' ? 'total threads' : 'total sessions'}
              </span>
            </article>
            <article className="dashboard-metric">
              <strong className="dashboard-metric-value">{usage.sessions7d}</strong>
              <span className="dashboard-metric-label">sessions, 7d</span>
            </article>
            <article className="dashboard-metric">
              <strong className="dashboard-metric-value">{usage.sessions30d}</strong>
              <span className="dashboard-metric-label">sessions, 30d</span>
            </article>
            <article className="dashboard-metric">
              <strong className="dashboard-metric-value">{usage.modelCount}</strong>
              <span className="dashboard-metric-label">models used</span>
            </article>
            <article className="dashboard-metric">
              <strong className="dashboard-metric-value">{compact(usage.inputTokens)}</strong>
              <span className="dashboard-metric-label">input tokens</span>
            </article>
            <article className="dashboard-metric">
              <strong className="dashboard-metric-value">{compact(usage.outputTokens)}</strong>
              <span className="dashboard-metric-label">output tokens</span>
            </article>
            <article className="dashboard-metric">
              <strong className="dashboard-metric-value">{compact(usage.cacheReadTokens)}</strong>
              <span className="dashboard-metric-label">cache read</span>
            </article>
            <article className="dashboard-metric">
              <strong className="dashboard-metric-value">{money(usage.totalCost)}</strong>
...
export function DashboardChartsSection({ daySeries }: DashboardChartsSectionProps) {
  const highestCount = daySeries.reduce((max, day) => Math.max(max, day.count), 1)
