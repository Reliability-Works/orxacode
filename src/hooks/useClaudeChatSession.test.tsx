...
function registerClaudeMessageTests() {
  ...
  it('reconciles provider state and history on mount', async () => {
    ...
    expect(window.orxa!.claudeChat.getSessionMessages).toHaveBeenCalledWith(
      'claude-thread-1',
      '/workspace'
    )
    expect(
      useUnifiedRuntimeStore.getState().claudeChatSessions[SESSION_KEY]?.historyMessages
    ).toHaveLength(1)
  })
}

function registerClaudeResumeTests() {
...
function registerClaudeStreamingTests() {
      ...
      events.emit({
        ...
        payload: {
          ...
          params: {
            id: 'assistant-2',
            turnId: 'turn-2',
            content: "I'll fan out a few agents.",
            timestamp: 3,
          },
        },
      })
...
function registerClaudeThinkingTests() {
...
function registerClaudeTaskTests() {
...
describe('useClaudeChatSession', () => {
  ...
  registerClaudeMessageTests()
  registerClaudeResumeTests()
  registerClaudeStreamingTests()
  registerClaudeThinkingTests()
  registerClaudeTaskTests()
})
