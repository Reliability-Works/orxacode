...
export function appendAssistantDeltaToLastMessage(prev: CodexMessageItem[], delta: string): CodexMessageItem[] {
  if (prev.length === 0) {
    return prev;
  }
  ...
  return plan.map((step: unknown, index: number) => {
    ...
    if (step && typeof step === "object") {
      ...
      const status: TodoItem["status"] =
        ...
        statusRaw === "in_progress" || statusRaw === "inProgress" ? "in_progress" :
