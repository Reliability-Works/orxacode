...
export function isSameGitDiffSnapshotEntry(
  left?: GitDiffSnapshotEntry,
  right?: GitDiffSnapshotEntry
) {
  if (!left || !right) {
    return false
  }
...
export function buildSyntheticCommandDiff(
  path: string,
  ...
  const type = beforeContent == null ? 'added' : afterContent == null ? 'deleted' : 'modified'
...
export function extractFileChangeDescriptors(
  item: {
    path?: string
    changeType?: unknown
    insertions?: number
