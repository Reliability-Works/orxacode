...
export type AppShellConfirmDialogRequest = Omit<ConfirmDialogProps, "isOpen" | "onConfirm" | "onCancel">;

export function useAppShellDialogs() {
  const [confirmDialogRequest, setConfirmDialogRequest] = useState<AppShellConfirmDialogRequest | null>(null);
  ...
  const submitTextInputDialog = useCallback((value: string) => {
