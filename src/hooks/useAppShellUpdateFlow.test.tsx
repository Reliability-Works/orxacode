...
describe("useAppShellUpdateFlow", () => {
  ...
  it("shows a temporary up-to-date message after a successful manual check with no update", () => {
    ...
    act(() => {
      result.current.handleUpdaterTelemetry({ phase: "check.start", manual: true, releaseChannel: "stable" });
    });

    expect(result.current.isCheckingForUpdates).toBe(true);

    act(() => {
      result.current.handleUpdaterTelemetry({ phase: "check.success", manual: true, releaseChannel: "stable", durationMs: 145 });
    });
  ...
  it("persists an update-found message when telemetry reports an available version", () => {
    ...
    act(() => {
      result.current.handleUpdaterTelemetry({ phase: "update.available", manual: false, releaseChannel: "stable", version: "0.1.0-beta.12" });
    });
