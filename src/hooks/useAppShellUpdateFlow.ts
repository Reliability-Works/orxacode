...
type UpdateFlowSetters = {
  clearStatusMessageTimer: () => void
  setAvailableUpdateVersion: (value: string | null) => void
  setIsCheckingForUpdates: (value: boolean) => void
  setStatusLine: (value: string) => void
  setUpdateInstallPending: (value: boolean) => void
  setUpdateProgressState: Dispatch<SetStateAction<AppShellUpdateProgressState | null>>
  setUpdateStatusMessage: Dispatch<SetStateAction<AppShellUpdateStatusMessage | null>>
  showTemporaryStatusMessage: (message: AppShellUpdateStatusMessage, timeoutMs?: number) => void
...
export function useAppShellUpdateFlow(input: UseAppShellUpdateFlowInput) {
  ...
  const [updateProgressState, setUpdateProgressState] =
    useState<AppShellUpdateProgressState | null>(null)
  const [updateStatusMessage, setUpdateStatusMessage] =
    useState<AppShellUpdateStatusMessage | null>(null)
  ...
  const showTemporaryStatusMessage = useCallback(
    (message: AppShellUpdateStatusMessage, timeoutMs = 2800) => {
  ...
  const handleUpdaterTelemetry = useCallback(
    (payload: UpdaterTelemetryPayload) => {
      applyUpdaterTelemetry(payload, {
        clearStatusMessageTimer,
        setAvailableUpdateVersion,
        setIsCheckingForUpdates,
        setStatusLine,
        setUpdateInstallPending,
  ...
  const downloadAndInstallUpdate = useCallback(async () => {
    await runDownloadAndInstallUpdate({
...
function applyUpdaterTelemetry(payload: UpdaterTelemetryPayload, setters: UpdateFlowSetters) {
  const {
    clearStatusMessageTimer,
    setAvailableUpdateVersion,
    setIsCheckingForUpdates,
  ...
  if (payload.phase === 'check.error') {
    ...
    if (updateInstallPending) {
      ...
      setUpdateProgressState({ phase: 'error', message: payload.message ?? 'Unable to update right now.' })
    }
    return
  }
  if (payload.phase === 'download.start') {
    clearStatusMessageTimer()
    setUpdateStatusMessage(null)
    setUpdateProgressState({ phase: 'downloading', message: 'Downloading update...', percent: 0, version: payload.version })
    return
  }
  if (payload.phase === 'download.progress') {
    setUpdateProgressState({ phase: 'downloading', message: 'Downloading update...', percent: payload.percent, version: payload.version })
    return
  }
  ...
  if (payload.phase === 'install.start') {
    ...
    setAvailableUpdateVersion(null)
    setUpdateStatusMessage(null)
    setUpdateProgressState({ phase: 'installing', message: 'Installing update...', percent: 100, version: payload.version })
...
async function runDownloadAndInstallUpdate({
  availableUpdateVersion,
  setStatusLine,
  setUpdateInstallPending,
  setUpdateProgressState,
  ...
  try {
    ...
    if (result.status === 'error') {
      setUpdateInstallPending(false)
      setUpdateProgressState({ phase: 'error', message: result.message ?? 'Unable to start update.' })
    } else if (result.status === 'skipped') {
      const detail = result.message ?? 'Unable to start update.'
      if (/already in progress/i.test(detail)) {
        setUpdateProgressState({ phase: 'downloading', message: 'Downloading update...', percent: undefined, version: availableUpdateVersion ?? undefined })
      } else {
        setUpdateInstallPending(false)
        setUpdateProgressState({ phase: 'error', message: detail })
      }
    } else {
      setUpdateProgressState({ phase: 'downloading', message: 'Downloading update...', percent: 0, version: availableUpdateVersion ?? undefined })
    }
    if (result.message) {
      setStatusLine(result.message)
    }
