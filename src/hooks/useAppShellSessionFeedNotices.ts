...
export function useAppShellSessionFeedNotices(input: UseAppShellSessionFeedNoticesInput) {
  const [sessionFeedNotices, setSessionFeedNotices] = useState<
    Record<string, AppShellSessionFeedNotice[]>
  >({})
  ...
  const addSessionFeedNotice = useCallback(
    (
      directory: string,
      sessionID: string,
      notice: Omit<AppShellSessionFeedNotice, 'id' | 'time'>
    ) => {
  ...
  const markManualSessionStopRequested = useCallback(
    (directory: string, sessionID: string, requestedAt = Date.now()) => {
  ...
  const markManualSessionStopNoticeEmitted = useCallback(
    (sessionKey: string, requestedAt = Date.now()) => {
