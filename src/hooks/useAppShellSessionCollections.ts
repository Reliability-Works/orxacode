...
export function useAppShellSessionCollections({
  ...
  pinnedSessions, archivedBackgroundAgentIds, hiddenBackgroundSessionIdsByProject, getSessionType,
  normalizePresentationProvider,
  ...
  const hiddenSessionIDsByProject = useMemo(() => {
    const projects = new Set([...Object.keys(archivedBackgroundAgentIds), ...Object.keys(hiddenBackgroundSessionIdsByProject)])
    const next: Record<string, string[]> = {}
    for (const directory of projects) {
      const ids = new Set([...(archivedBackgroundAgentIds[directory] ?? []), ...(hiddenBackgroundSessionIdsByProject[directory] ?? [])])
      next[directory] = [...ids]
    }
    return next
