...
export function useAppShellCommitFlow<NextStep extends string>(input: UseAppShellCommitFlowInput<NextStep>) {
  const { runningMessage, successMessage } = input;
  const [commitFlowState, setCommitFlowState] = useState<AppShellCommitFlowState<NextStep> | null>(null);
  ...
  const scheduleCommitFlowDismiss = useCallback((delayMs: number) => {
    clearCommitFlowDismissTimer();
  ...
  const startCommitFlow = useCallback((nextStep: NextStep) => {
    setCommitFlowState({
      phase: "running",
      nextStep,
  ...
  const completeCommitFlow = useCallback((nextStep: NextStep) => {
    setCommitFlowState({
      phase: "success",
      nextStep,
  ...
  const failCommitFlow = useCallback((nextStep: NextStep, message: string) => {
    setCommitFlowState({
      phase: "error",
      nextStep,
      message,
