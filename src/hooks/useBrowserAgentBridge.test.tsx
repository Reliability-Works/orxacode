...
import {
  ...
  parseBrowserActionsFromText,
  useBrowserAgentBridge,
} from './useBrowserAgentBridge'
...
describe('useBrowserAgentBridge action dispatch', () => {
  it('executes actions once per id and returns machine results', async () => {
    ...
    const messages = [
      assistantBundle(
        '<orxa_browser_action>{"id":"action-1","action":"navigate","args":{"url":"https://example.com"}}</orxa_browser_action>'
      ),
      assistantBundle(
        '<orxa_browser_action>{"id":"action-1","action":"navigate","args":{"url":"https://example.com"}}</orxa_browser_action>'
      ),
  ...
  it('blocks actions when control owner is human without injecting machine prompts', async () => {
    ...
    renderHook(() =>
      useBrowserAgentBridge({
        ...
        messages: [
          assistantBundle(
            '<orxa_browser_action>{"id":"action-2","action":"click","args":{"selector":"#buy"}}</orxa_browser_action>'
          ),
        ],
        browserModeEnabled: true,
        controlOwner: 'human',
...
describe('useBrowserAgentBridge replay guards', () => {
        ...
        messages: [
          ...
          ),
          {
            ...
            parts: [
              {
                type: 'text',
                text: '[ORXA_BROWSER_RESULT]{"id":"action-7","action":"navigate","ok":true}',
              },
            ],
          } as unknown as SessionMessageBundle,
        ],
  ...
  it('does not execute browser actions while automation is halted for the session', async () => {
    ...
    renderHook(() =>
      useBrowserAgentBridge({
        ...
        messages: [
          assistantBundle(
            '<orxa_browser_action>{"id":"action-8","action":"navigate","args":{"url":"https://example.com"}}</orxa_browser_action>'
          ),
        ],
        browserModeEnabled: true,
        controlOwner: 'agent',
...
describe('useBrowserAgentBridge result payloads', () => {
  it('attaches screenshot artifacts when browser action returns file metadata', async () => {
    ...
    renderHook(() =>
      useBrowserAgentBridge({
        ...
        messages: [
          assistantBundle(
            '<orxa_browser_action>{"id":"action-3","action":"screenshot","args":{"tabID":"tab-1"}}</orxa_browser_action>'
          ),
        ],
        browserModeEnabled: true,
        controlOwner: 'agent',
...
describe('useBrowserAgentBridge callback stability', () => {
  it('does not replay the same action when status callback identity changes', async () => {
    ...
    const messages = [
      assistantBundle(
        '<orxa_browser_action>{"id":"action-5","action":"navigate","args":{"url":"https://example.com"}}</orxa_browser_action>'
      ),
    ]
  ...
  it('raises guardrail violation when assistant text references forbidden external tooling', async () => {
    ...
    renderHook(() =>
      useBrowserAgentBridge({
        ...
        messages: [
          assistantBundle(
            'Error: McpError: MCP error -32603: failed to connect to running Pencil app.'
          ),
        ],
        browserModeEnabled: true,
        controlOwner: 'agent',
...
describe('useBrowserAgentBridge guardrails', () => {
  it('raises guardrail violation when web progress is claimed without ORXA browser actions', async () => {
    ...
    renderHook(() =>
      useBrowserAgentBridge({
        ...
        activeSessionID: 'session-1',
        messages: [
          assistantBundle("Great start! I've loaded the a16z report and captured key points."),
        ],
        browserModeEnabled: true,
        controlOwner: 'agent',
        onGuardrailViolation,
  ...
  it('does not raise progress guardrail when ORXA browser action tags are present', async () => {
    ...
    renderHook(() =>
      useBrowserAgentBridge({
        ...
        messages: [
          assistantBundle(
            '<orxa_browser_action>{"id":"action-6","action":"navigate","args":{"url":"https://example.com"}}</orxa_browser_action>'
          ),
