...
describe("message-feed-visibility helpers", () => {
  it("filters user-visible parts and keeps attachments when user text is visible", () => {
    const parts = [
      ...
      { id: "text-internal", type: "text", text: '[ORXA_BROWSER_RESULT]{"action":"navigate","ok":true}' },
  ...
  it("hides machine/internal assistant text payloads and keeps normal prose", () => {
    expect(
      shouldHideAssistantText('<orxa_browser_action>{"id":"action-1","action":"navigate"}</orxa_browser_action>'),
    ).toBe(true);
    expect(shouldHideAssistantText('{"type":"step-start","sessionID":"s1","messageID":"m1"}')).toBe(true);
    expect(shouldHideAssistantText("Captured first source. Continuing evidence collection.")).toBe(false);
  });
