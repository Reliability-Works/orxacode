...
function getBridge() {
  if (!window.orxa?.opencode) {
    throw new OpencodeClientError(
      'Desktop bridge unavailable. Restart Orxa Code to reconnect.',
      'IPC_UNAVAILABLE',
      false
    )
  }
...
export const opencodeClient = {
  ...
  async createSession(
    ...
    return withRetry(
      'createSession',
      () => getBridge().createSession(directory, title, permissionMode),
      { retries: 0 }
    )
  },
  ...
  async loadMessages(directory: string, sessionID: string): Promise<SessionMessageBundle[]> {
    return withRetry('loadMessages', () => getBridge().loadMessages(directory, sessionID), {
      retries: 2,
    })
  },
  ...
  async getSessionRuntime(directory: string, sessionID: string): Promise<SessionRuntimeSnapshot> {
    return withRetry(
      'getSessionRuntime',
      () => getBridge().getSessionRuntime(directory, sessionID),
      { retries: 1 }
    )
  },
  ...
  async loadExecutionLedger(
    ...
    return withRetry(
      'loadExecutionLedger',
      () => getBridge().loadExecutionLedger(directory, sessionID, cursor),
      { retries: 1 }
    )
  },
  ...
  async loadChangeProvenance(
    ...
    return withRetry(
      ...
      () => getBridge().loadChangeProvenance(directory, sessionID, cursor),
      { retries: 1 }
    )
  },
  ...
  async getFileProvenance(
    ...
    return withRetry(
      'getFileProvenance',
      () => getBridge().getFileProvenance(directory, sessionID, relativePath),
      { retries: 1 }
    )
  },
  ...
  async listArtifactSessions(workspace: string): Promise<ArtifactSessionSummary[]> {
    return withRetry('listArtifactSessions', () => getBridge().listArtifactSessions(workspace), {
      retries: 1,
    })
  },
  ...
  async listWorkspaceArtifactSummary(workspace: string): Promise<WorkspaceArtifactSummary> {
    return withRetry(
      'listWorkspaceArtifactSummary',
      () => getBridge().listWorkspaceArtifactSummary(workspace),
      { retries: 1 }
    )
  },

  async getArtifactRetentionPolicy(): Promise<ArtifactRetentionPolicy> {
    return withRetry('getArtifactRetentionPolicy', () => getBridge().getArtifactRetentionPolicy(), {
      retries: 1,
    })
  },
  ...
  async setArtifactRetentionPolicy(
    ...
    return withRetry(
      'setArtifactRetentionPolicy',
      () => getBridge().setArtifactRetentionPolicy(input),
      { retries: 0 }
    )
  },

  async pruneArtifactsNow(workspace?: string): Promise<ArtifactPruneResult> {
    return withRetry('pruneArtifactsNow', () => getBridge().pruneArtifactsNow(workspace), {
      retries: 0,
    })
  },
  ...
  async exportArtifactBundle(
    ...
    return withRetry('exportArtifactBundle', () => getBridge().exportArtifactBundle(input), {
      retries: 0,
    })
  },

  async listWorkspaceContext(workspace: string): Promise<WorkspaceContextFile[]> {
    return withRetry('listWorkspaceContext', () => getBridge().listWorkspaceContext(workspace), {
      retries: 1,
    })
  },
  ...
  async readWorkspaceContext(workspace: string, id: string): Promise<WorkspaceContextFile> {
    return withRetry(
      ...
      () => getBridge().readWorkspaceContext(workspace, id),
      { retries: 1 }
    )
  },

  async writeWorkspaceContext(input: WorkspaceContextWriteInput): Promise<WorkspaceContextFile> {
    return withRetry('writeWorkspaceContext', () => getBridge().writeWorkspaceContext(input), {
      retries: 0,
    })
  },
  ...
  async gitCheckoutBranch(directory: string, branch: string): Promise<void> {
    await withRetry('gitCheckoutBranch', () => getBridge().gitCheckoutBranch(directory, branch), {
      retries: 0,
    })
  },
}
