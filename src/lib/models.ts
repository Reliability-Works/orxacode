...
): Array<{ id: string; name: string; models: ProviderModelLike[] }> {
...
function createModelOption(
  ...
  const resolvedModelID =
    typeof model.id === 'string' && model.id.trim().length > 0 ? model.id : modelID
  const modelName =
    typeof model.name === 'string' && model.name.trim().length > 0 ? model.name : resolvedModelID
  const variants =
...
function collectConfiguredProviderID(ids: Set<string>, value: unknown) {
...
export function filterModelOptionsByProviderIDs(
  options: ModelOption[],
  ...
  for (const provider of listProviderEntries(providers)) {
    ...
    for (const model of provider.models) {
      ...
      options.push({
        ...
        modelName:
          typeof model.name === 'string' && model.name.trim().length > 0 ? model.name : model.id,
...
function processArrayModels(
  providerID: string,
  providerName: string,
  models: unknown[],
  pushOption: (option: ModelOption) => void
): void {
  for (const modelValue of models) {
    ...
    const parsed = createModelOption(providerID, providerName, fallbackModelID, item)
    if (parsed) {
      pushOption(parsed)
    }
  }
...
function processObjectModels(
  ...
  for (const [modelID, modelValue] of Object.entries(models)) {
    ...
    const parsed = createModelOption(providerID, providerName, fallbackModelID, modelValue)
    if (parsed) {
      pushOption(parsed)
    }
  }
...
export function listModelOptionsFromConfig(config: unknown): ModelOption[] {
  ...
  for (const [providerID, providerValue] of Object.entries(providers)) {
    ...
    if (Array.isArray(models)) {
      processArrayModels(providerID, providerName, models, pushOption)
      continue
    }

    if (typeof models === 'object') {
      processObjectModels(providerID, providerName, models as Record<string, unknown>, pushOption)
    }
  }
...
function collectProviderIDsFromRoot(root: Record<string, unknown>, ids: Set<string>): void {
  const providers = root.provider ?? root.providers
  if (providers && typeof providers === 'object' && !Array.isArray(providers)) {
  ...
  collectConfiguredProviderID(ids, root.model)
  collectConfiguredProviderID(ids, root.small_model)
}

function collectProviderIDsFromNested(
  obj: unknown,
  ids: Set<string>,
  key: 'model' | 'models'
): void {
  ...
  collectConfiguredProviderID(ids, (obj as Record<string, unknown>)[key])
}

function collectProviderIDsFromAgents(root: Record<string, unknown>, ids: Set<string>): void {
  ...
  for (const definition of Object.values(agents)) {
    ...
    collectConfiguredProviderID(ids, (definition as { model?: unknown }).model)
  }
}
...
export function listConfiguredProviderIDs(config: unknown): string[] {
  ...
  collectProviderIDsFromRoot(root, ids)
  collectProviderIDsFromNested(root.orxa, ids, 'model')
  collectProviderIDsFromNested(root.plan, ids, 'model')
  collectProviderIDsFromAgents(root, ids)
