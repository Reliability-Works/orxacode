...
function writeDesktopEnvelope(filePath: string) {
  return fsWriteDesktopEnvelope(filePath)
}

function fsWriteDesktopEnvelope(filePath: string) {
...
function inheritedFdFallbackEffect() {
  return Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem
    const filePath = yield* fs.makeTempFileScoped({ prefix: 'orxa-bootstrap-', suffix: '.ndjson' })
...
function timeoutBeforeValueEffect() {
  return Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem
    const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: 'orxa-bootstrap-' })
...
it.layer(NodeServices.layer)('readBootstrapEnvelope', it => {
  ...
  it.effect('reads a bootstrap envelope from a provided fd', () => providedFdEnvelopeEffect())

  it.effect('falls back to reading the inherited fd when path duplication fails', () =>
    inheritedFdFallbackEffect()
  )

  it.effect('returns none when the fd is unavailable', () => unavailableFdEffect())

  it.effect('returns none when the bootstrap read times out before any value arrives', () =>
    timeoutBeforeValueEffect()
  )
})
