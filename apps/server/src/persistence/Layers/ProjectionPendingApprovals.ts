...
function makeUpsertProjectionPendingApprovalRow(sql: SqlClient.SqlClient) {
...
function makeListProjectionPendingApprovalRows(sql: SqlClient.SqlClient) {
...
function makeGetProjectionPendingApprovalRow(sql: SqlClient.SqlClient) {
...
function makeDeleteProjectionPendingApprovalRow(sql: SqlClient.SqlClient) {
  return SqlSchema.void({
...
const makeProjectionPendingApprovalRepository = Effect.gen(function* () {
  ...
  const upsertProjectionPendingApprovalRow = makeUpsertProjectionPendingApprovalRow(sql)
  const listProjectionPendingApprovalRows = makeListProjectionPendingApprovalRows(sql)
  const getProjectionPendingApprovalRow = makeGetProjectionPendingApprovalRow(sql)
  const deleteProjectionPendingApprovalRow = makeDeleteProjectionPendingApprovalRow(sql)
  ...
  const deleteByRequestId: ProjectionPendingApprovalRepositoryShape['deleteByRequestId'] = input =>
