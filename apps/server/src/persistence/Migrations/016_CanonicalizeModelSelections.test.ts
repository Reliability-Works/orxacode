...
const expectedCanonicalizedEventPayloads = [
  {
    projectId: 'project-1',
    title: 'Project',
...
function seedLegacyProjects(sql: SqlClient.SqlClient) {
...
function seedLegacyThreads(sql: SqlClient.SqlClient) {
  return sql`
...
function seedLegacySession(sql: SqlClient.SqlClient) {
  return sql`
...
function seedLegacyEvents(sql: SqlClient.SqlClient) {
  return sql.unsafe(legacyEventInsertSql)
}

function seedLegacyProjectionState(sql: SqlClient.SqlClient) {
  return Effect.gen(function* () {
    yield* runMigrations({ toMigrationInclusive: 15 })
    yield* seedLegacyProjects(sql)
    yield* seedLegacyThreads(sql)
    yield* seedLegacySession(sql)
    yield* seedLegacyEvents(sql)
  })
}

function assertCanonicalizedProjects(sql: SqlClient.SqlClient) {
  return Effect.gen(function* () {
...
function assertCanonicalizedThreads(sql: SqlClient.SqlClient) {
  return Effect.gen(function* () {
...
function assertCanonicalizedEvents(sql: SqlClient.SqlClient) {
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(
      eventRows.map(row => JSON.parse(row.payloadJson)),
      [...expectedCanonicalizedEventPayloads]
    )
  })
}

function assertCanonicalizedProjectionState(sql: SqlClient.SqlClient) {
  return Effect.gen(function* () {
    yield* assertCanonicalizedProjects(sql)
    yield* assertCanonicalizedThreads(sql)
    yield* assertCanonicalizedEvents(sql)
  })
}
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* seedLegacyProjectionState(sql)
        yield* runMigrations({ toMigrationInclusive: 16 })
        yield* assertCanonicalizedProjectionState(sql)
      })
  )
})
