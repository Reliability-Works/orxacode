...
async function readFirstPromptValue(
  input:
    | {
        readonly prompt: AsyncIterable<SDKUserMessage>
      }
...
export async function readFirstPromptText(
  ...
  const value = await readFirstPromptValue(input)
  if (!value) {
    return undefined
  }
...
export async function readFirstPromptMessage(
  ...
  return readFirstPromptValue(input)
}
...
export type ClaudeAdapterService = typeof ClaudeAdapter.Service
