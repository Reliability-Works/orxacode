...
it.layer(makeDirectoryLayer(SqlitePersistenceMemory))(
  'ProviderSessionDirectoryLive bindings',
  it => {
...
it.layer(makeDirectoryLayer(SqlitePersistenceMemory))(
  'ProviderSessionDirectoryLive provider changes',
  it => {
...
it.layer(makeDirectoryLayer(SqlitePersistenceMemory))(
  'ProviderSessionDirectoryLive restart rehydration',
  it => {
    it('rehydrates persisted mappings across layer restart', () =>
      Effect.gen(function* () {
        const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'orxa-provider-directory-'))
