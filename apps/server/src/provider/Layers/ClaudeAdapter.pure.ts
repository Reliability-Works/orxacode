/**
 * Pure helpers for the Claude adapter — primitive utilities and prompt
 * building.
 *
 * Stateless helpers used by the layer wiring shell and sibling helper
 * modules. This file owns small primitives (identity guards, error
 * shaping, token usage normalization, tool classification, prompt
 * construction). Larger SDK/message helpers live in `ClaudeAdapter.sdk.ts`.
 *
 * @module ClaudeAdapter.pure
 */
import type {
  SDKResultMessage,
  SDKUserMessage,
...
export function asCanonicalTurnId<T>(value: T): T {
  return value
}
...
export function normalizeClaudeTokenUsage(
  ...
  const directUsedTokens = readFiniteNumber(record, 'total_tokens')
  const inputTokens =
    sumFiniteNumbers(record, [
      'input_tokens',
      'cache_creation_input_tokens',
      'cache_read_input_tokens',
    ]) ?? 0
  const outputTokens = readFiniteNumber(record, 'output_tokens') ?? 0
  ...
  const maxTokens =
    typeof contextWindow === 'number' && Number.isFinite(contextWindow) && contextWindow > 0
      ? contextWindow
      : undefined
  const toolUses = readFiniteNumber(record, 'tool_uses')
  const durationMs = readFiniteNumber(record, 'duration_ms')
...
export function readClaudeResumeState(
  resumeCursor: unknown
): import('./ClaudeAdapter.types.ts').ClaudeResumeState | undefined {
...
const CLAUDE_COLLAB_TOOL_MARKERS = ['agent', 'subagent', 'sub-agent'] as const
const CLAUDE_COMMAND_TOOL_MARKERS = ['bash', 'command', 'shell', 'terminal'] as const
const CLAUDE_FILE_CHANGE_TOOL_MARKERS = [
  'edit',
  'write',
  'file',
  'patch',
...
export function classifyToolItemType(toolName: string): CanonicalItemType {
  ...
  if (normalized === 'task' || includesAnyMarker(normalized, CLAUDE_COLLAB_TOOL_MARKERS)) {
    return 'collab_agent_tool_call'
  }
  if (includesAnyMarker(normalized, CLAUDE_COMMAND_TOOL_MARKERS)) {
    return 'command_execution'
  }
  if (includesAnyMarker(normalized, CLAUDE_FILE_CHANGE_TOOL_MARKERS)) {
    return 'file_change'
  }
