import { KeybindingCommand, KeybindingRule, KeybindingsConfig } from '@orxa-code/contracts'
...
import { KeybindingsConfigError } from '@orxa-code/contracts'
...
it.layer(NodeServices.layer)('keybindings parsing shortcuts', it => {
...
it.layer(NodeServices.layer)('keybindings parsing encoding', it => {
...
it.layer(NodeServices.layer)('keybindings defaults', it => {
...
it.layer(NodeServices.layer)('keybindings config loading', it => {
...
it.layer(NodeServices.layer)('keybindings startup sync', it => {
  it.effect(
  ...
  it.effect('skips conflicting default keybindings on startup and logs a detailed warning', () => {
    ...
    return Effect.gen(function* () {
      ...
      Effect.provide(
        Layer.mergeAll(makeKeybindingsLayer(), Logger.layer([logger], { mergeWithExisting: false }))
      )
    )
  })
})

it.layer(NodeServices.layer)('keybindings writes success', it => {
...
it.layer(NodeServices.layer)('keybindings writes failure', it => {
...
it.layer(NodeServices.layer)('keybindings cache and concurrency', it => {
