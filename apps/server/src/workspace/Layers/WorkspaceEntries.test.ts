...
const TestLayer = Layer.empty.pipe(
  ...
  Layer.provide(
    ServerConfig.layerTest(process.cwd(), {
      prefix: 'orxa-workspace-entries-test-',
    })
  ),
...
const makeTempDir = Effect.fn(function* (opts?: { prefix?: string; git?: boolean }) {
  ...
  const dir = yield* fileSystem.makeTempDirectoryScoped({
    prefix: opts?.prefix ?? 'orxa-workspace-entries-',
  })
  if (opts?.git) {
...
it.layer(TestLayer)('WorkspaceEntriesLive search basics', it => {
  describe('search', () => {
    ...
    it.effect('filters and ranks entries by query', () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: 'orxa-workspace-query-' })
        yield* writeTextFile(cwd, 'src/components/Composer.tsx')
...
it.layer(TestLayer)('WorkspaceEntriesLive search ignore handling', it => {
  describe('search', () => {
    it.effect('supports fuzzy subsequence queries for composer path search', () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: 'orxa-workspace-fuzzy-query-' })
        yield* writeTextFile(cwd, 'src/components/Composer.tsx')
        yield* writeTextFile(cwd, 'src/components/composePrompt.ts')
        yield* writeTextFile(cwd, 'docs/composition.md')
    ...
    it.effect('tracks truncation without sorting every fuzzy match', () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: 'orxa-workspace-fuzzy-limit-' })
        yield* writeTextFile(cwd, 'src/components/Composer.tsx')
        yield* writeTextFile(cwd, 'src/components/composePrompt.ts')
        yield* writeTextFile(cwd, 'docs/composition.md')
    ...
    it.effect('excludes gitignored paths for git repositories', () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: 'orxa-workspace-gitignore-', git: true })
    ...
    it.effect('excludes tracked paths that match ignore rules', () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({
          prefix: 'orxa-workspace-tracked-gitignore-',
          git: true,
        })
        yield* writeTextFile(cwd, '.convex/local-storage/data.json', '{}')
        yield* writeTextFile(cwd, 'src/keep.ts', 'export {};')
    ...
    it.effect('excludes .convex in non-git workspaces', () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: 'orxa-workspace-non-git-convex-' })
        yield* writeTextFile(cwd, '.convex/local-storage/data.json', '{}')
        yield* writeTextFile(cwd, 'src/keep.ts', 'export {};')
...
it.layer(TestLayer)('WorkspaceEntriesLive search concurrency', it => {
  describe('search', () => {
    it.effect('deduplicates concurrent index builds for the same cwd', () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: 'orxa-workspace-concurrent-build-' })
        yield* writeTextFile(cwd, 'src/components/Composer.tsx')
    ...
    it.effect('limits concurrent directory reads while walking the filesystem', () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: 'orxa-workspace-read-concurrency-' })
