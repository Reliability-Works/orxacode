...
interface WorkspaceEntriesRuntimeDeps {
  readonly path: Path.Path
  readonly gitOption: Option.Option<GitCoreShape>
  readonly workspacePaths: WorkspacePathsShape
}

interface WorkspaceReadDirectoryResult {
  readonly relativeDir: string
  readonly dirents: Dirent[] | null
}

interface WorkspaceCandidateEntry {
...
function filterGitIgnoredPaths(
  gitOption: Option.Option<GitCoreShape>,
  cwd: string,
...
function readDirectoryEntries(
  ...
  cwd: string,
  relativeDir: string
): Effect.Effect<WorkspaceReadDirectoryResult, WorkspaceEntriesError> {
...
function appendWorkspaceCandidates(input: {
  candidateEntriesByDirectory: ReadonlyArray<ReadonlyArray<WorkspaceCandidateEntry>>
  allowedPathSet: ReadonlySet<string> | null
  entries: SearchableWorkspaceEntry[]
  pendingDirectories: string[]
}): boolean {
  let truncated = false

  for (const candidateEntries of input.candidateEntriesByDirectory) {
    for (const candidate of candidateEntries) {
      if (input.allowedPathSet && !input.allowedPathSet.has(candidate.relativePath)) {
        continue
      }
      ...
      if (candidate.dirent.isDirectory()) {
        input.pendingDirectories.push(candidate.relativePath)
      }

      if (input.entries.length >= WORKSPACE_INDEX_MAX_ENTRIES) {
        truncated = true
        break
      }
    }
...
function buildWorkspaceIndexFromGit(
  deps: Pick<WorkspaceEntriesRuntimeDeps, 'gitOption'>,
  cwd: string
): Effect.Effect<WorkspaceIndex | null> {
  return Effect.gen(function* () {
    if (Option.isNone(deps.gitOption)) {
      return null
    }
    if (!(yield* isInsideGitWorkTree(deps.gitOption, cwd))) {
      return null
    }

    const listedFiles = yield* deps.gitOption.value
    ...
    const filePaths = yield* filterGitIgnoredPaths(deps.gitOption, cwd, listedPaths)
...
function buildWorkspaceIndexFromFilesystem(
  deps: Pick<WorkspaceEntriesRuntimeDeps, 'path' | 'gitOption'>,
  cwd: string
): Effect.Effect<WorkspaceIndex, WorkspaceEntriesError> {
  return Effect.gen(function* () {
    const shouldFilterWithGitIgnore = yield* isInsideGitWorkTree(deps.gitOption, cwd)
    ...
    while (pendingDirectories.length > 0 && !truncated) {
      ...
      const directoryEntries = yield* Effect.forEach(
        currentDirectories,
        relativeDir => readDirectoryEntries(deps.path, cwd, relativeDir),
        { concurrency: WORKSPACE_SCAN_READDIR_CONCURRENCY }
      )

      const candidateEntriesByDirectory = directoryEntries.map(directoryEntry =>
        collectDirectoryCandidates(deps.path, directoryEntry.relativeDir, directoryEntry.dirents)
      )
      ...
      const allowedPathSet = shouldFilterWithGitIgnore
        ? new Set(yield* filterGitIgnoredPaths(deps.gitOption, cwd, candidatePaths))
        : null

      truncated = appendWorkspaceCandidates({
        candidateEntriesByDirectory,
        allowedPathSet,
        entries,
        pendingDirectories,
      })
...
function buildWorkspaceIndex(
  ...
  return Effect.gen(function* () {
    const gitIndexed = yield* buildWorkspaceIndexFromGit(deps, cwd)
    if (gitIndexed) {
      return gitIndexed
    }
    return yield* buildWorkspaceIndexFromFilesystem(deps, cwd)
  })
}
...
function normalizeWorkspaceRoot(
  ...
  cwd: string
): Effect.Effect<string, WorkspaceEntriesError> {
...
function invalidateWorkspaceEntries(
  workspacePaths: WorkspacePathsShape,
  workspaceIndexCache: Cache.Cache<string, WorkspaceIndex, WorkspaceEntriesError>,
  cwd: string
): Effect.Effect<void> {
  return Effect.gen(function* () {
    const normalizedCwd = yield* normalizeWorkspaceRoot(workspacePaths, cwd).pipe(
...
function searchWorkspaceIndex(
  ...
  { entries: SearchableWorkspaceEntry[]; truncated: boolean },
  WorkspaceEntriesError
> {
  return Effect.gen(function* () {
    const normalizedCwd = yield* normalizeWorkspaceRoot(workspacePaths, input.cwd)
...
export const makeWorkspaceEntries = Effect.gen(function* () {
  ...
  const runtimeDeps = { path, gitOption, workspacePaths } satisfies WorkspaceEntriesRuntimeDeps
  ...
  const workspaceIndexCache = yield* Cache.makeWith<string, WorkspaceIndex, WorkspaceEntriesError>({
    ...
    lookup: cwd => buildWorkspaceIndex(runtimeDeps, cwd),
    timeToLive: exit =>
