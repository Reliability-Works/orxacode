...
import { ServerLifecycleEvents, type ServerLifecycleEventsShape } from './serverLifecycleEvents'
...
const startKeybindingsRuntime = (keybindings: KeybindingsShape) =>
...
const startServerSettingsRuntime = (serverSettings: ServerSettingsShape) =>
...
const publishWelcomeEvent = (lifecycleEvents: ServerLifecycleEventsShape) =>
  Effect.gen(function* () {
...
const handleStartupFailure = (commandGate: CommandGate, startupExit: Exit.Exit<void, unknown>) =>
  Effect.gen(function* () {
...
const publishReadyEvent = (
  lifecycleEvents: ServerLifecycleEventsShape,
  httpListening: Deferred.Deferred<void>
) =>
  Effect.gen(function* () {
...
const makeServerRuntimeStartup = Effect.gen(function* () {
  ...
  const startup = Effect.gen(function* () {
    ...
    yield* startKeybindingsRuntime(keybindings)

    yield* Effect.logDebug('startup phase: starting server settings runtime')
    yield* startServerSettingsRuntime(serverSettings)

    yield* Effect.logDebug('startup phase: starting orchestration reactors')
    yield* orchestrationReactor.start().pipe(Scope.provide(reactorScope))

    yield* publishWelcomeEvent(lifecycleEvents)
  })
  ...
  yield* Effect.forkScoped(
    Effect.gen(function* () {
      ...
      if (Exit.isFailure(startupExit)) {
        yield* handleStartupFailure(commandGate, startupExit)
        return
      }

      yield* Effect.logDebug('Accepting commands')
      yield* commandGate.signalCommandReady
      yield* publishReadyEvent(lifecycleEvents, httpListening)
...
export const ServerRuntimeStartupLive = Layer.effect(ServerRuntimeStartup, makeServerRuntimeStartup)
