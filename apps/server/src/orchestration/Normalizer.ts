import { Effect, FileSystem, Path } from 'effect'
import {
  type ClientOrchestrationCommand,
  type OrchestrationCommand,
  OrchestrationDispatchCommandError,
  PROVIDER_SEND_TURN_MAX_IMAGE_BYTES,
} from '@orxa-code/contracts'

import { createAttachmentId, resolveAttachmentPath } from '../attachmentStore'
import { ServerConfig } from '../config'
import { parseBase64DataUrl } from '../imageMime'
import { WorkspacePaths } from '../workspace/Services/WorkspacePaths'

const toDispatchCommandError = (message: string) =>
  new OrchestrationDispatchCommandError({ message })

const normalizeProjectWorkspaceRoot = (
  workspacePaths: typeof WorkspacePaths.Service,
  workspaceRoot: string
) =>
  workspacePaths
    .normalizeWorkspaceRoot(workspaceRoot)
    .pipe(Effect.mapError(cause => toDispatchCommandError(cause.message)))

const persistTurnStartAttachment = ({
  attachment,
  threadId,
  attachmentsDir,
  fileSystem,
  path,
}: {
  attachment: Extract<
    ClientOrchestrationCommand,
    { type: 'thread.turn.start' }
  >['message']['attachments'][number]
  threadId: string
  attachmentsDir: string
  fileSystem: typeof FileSystem.FileSystem.Service
  path: typeof Path.Path.Service
}) =>
  Effect.gen(function* () {
    const parsed = parseBase64DataUrl(attachment.dataUrl)
    if (!parsed || !parsed.mimeType.startsWith('image/')) {
      return yield* toDispatchCommandError(
        `Invalid image attachment payload for '${attachment.name}'.`
      )
    }

    const bytes = Buffer.from(parsed.base64, 'base64')
    if (bytes.byteLength === 0 || bytes.byteLength > PROVIDER_SEND_TURN_MAX_IMAGE_BYTES) {
      return yield* toDispatchCommandError(
        `Image attachment '${attachment.name}' is empty or too large.`
      )
    }

    const attachmentId = createAttachmentId(threadId)
    if (!attachmentId) {
      return yield* toDispatchCommandError('Failed to create a safe attachment id.')
    }

    const persistedAttachment = {
      type: 'image' as const,
      id: attachmentId,
      name: attachment.name,
      mimeType: parsed.mimeType.toLowerCase(),
      sizeBytes: bytes.byteLength,
    }

    const attachmentPath = resolveAttachmentPath({
      attachmentsDir,
      attachment: persistedAttachment,
    })
    if (!attachmentPath) {
      return yield* toDispatchCommandError(
        `Failed to resolve persisted path for '${attachment.name}'.`
      )
    }

    yield* fileSystem
      .makeDirectory(path.dirname(attachmentPath), { recursive: true })
      .pipe(
        Effect.mapError(() =>
          toDispatchCommandError(`Failed to create attachment directory for '${attachment.name}'.`)
        )
      )
    yield* fileSystem
      .writeFile(attachmentPath, bytes)
      .pipe(
        Effect.mapError(() =>
          toDispatchCommandError(`Failed to persist attachment '${attachment.name}'.`)
        )
      )

    return persistedAttachment
  })

const normalizeThreadTurnStartCommand = ({
  command,
  attachmentsDir,
  fileSystem,
  path,
}: {
  command: Extract<ClientOrchestrationCommand, { type: 'thread.turn.start' }>
  attachmentsDir: string
  fileSystem: typeof FileSystem.FileSystem.Service
  path: typeof Path.Path.Service
}) =>
  Effect.gen(function* () {
    const normalizedAttachments = yield* Effect.forEach(
      command.message.attachments,
      attachment =>
        persistTurnStartAttachment({
          attachment,
          threadId: command.threadId,
          attachmentsDir,
          fileSystem,
          path,
        }),
      { concurrency: 1 }
    )

    return {
      ...command,
      message: {
        ...command.message,
        attachments: normalizedAttachments,
      },
    } satisfies OrchestrationCommand
  })

export const normalizeDispatchCommand = (command: ClientOrchestrationCommand) =>
  Effect.gen(function* () {
    const fileSystem = yield* FileSystem.FileSystem
    const path = yield* Path.Path
    const serverConfig = yield* ServerConfig
    const workspacePaths = yield* WorkspacePaths

    if (command.type === 'project.create') {
      return {
        ...command,
        workspaceRoot: yield* normalizeProjectWorkspaceRoot(workspacePaths, command.workspaceRoot),
      } satisfies OrchestrationCommand
    }

    if (command.type === 'project.meta.update' && command.workspaceRoot !== undefined) {
      return {
        ...command,
        workspaceRoot: yield* normalizeProjectWorkspaceRoot(workspacePaths, command.workspaceRoot),
      } satisfies OrchestrationCommand
    }

    if (command.type !== 'thread.turn.start') {
      return command as OrchestrationCommand
    }

    return yield* normalizeThreadTurnStartCommand({
      command,
      attachmentsDir: serverConfig.attachmentsDir,
      fileSystem,
      path,
    })
  })
