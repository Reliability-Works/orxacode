...
type ResolvedBinding = ResolvedKeybindingsConfig[number] & {}

function forEachContextualBinding(
  keybindings: ResolvedKeybindingsConfig,
  context: ShortcutMatchContext,
  callback: (binding: ResolvedBinding) => boolean | void
): void {
  for (let index = keybindings.length - 1; index >= 0; index -= 1) {
    const binding = keybindings[index]
    if (!binding) continue
    if (!matchesWhenClause(binding.whenAst, context)) continue
    if (callback(binding) === false) return
  }
}
...
function findEffectiveShortcutForCommand(
  ...
  let result: KeybindingShortcut | null = null

  forEachContextualBinding(keybindings, context, binding => {
    const conflictKey = shortcutConflictKey(binding.shortcut, platform)
    if (claimedShortcuts.has(conflictKey)) return
...
export function resolveShortcutCommand(
  ...
  const platform = resolvePlatform(options)
  const context = resolveContext(options)
  let result: KeybindingCommand | null = null

  forEachContextualBinding(keybindings, context, binding => {
    if (!matchesShortcut(event, binding.shortcut, platform)) return
...
function resolveMacTerminalNavigationCommand(
  event: ShortcutEventLike,
  moveWord: string,
  moveLine: string
) {
...
function resolveDefaultTerminalNavigationCommand(event: ShortcutEventLike, moveWord: string) {
...
export function terminalNavigationShortcutData(
  ...
  if (isMacPlatform(platform)) {
    return resolveMacTerminalNavigationCommand(event, moveWord, moveLine)
  }

  return resolveDefaultTerminalNavigationCommand(event, moveWord)
}
