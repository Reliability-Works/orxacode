...
const INITIAL_WS_CONNECTION_STATUS: WsConnectionStatus = Object.freeze({
  attemptCount: 0,
  closeCode: null,
  closeReason: null,
  connectedAt: null,
...
function updateWsConnectionStatus(
  ...
  useWsConnectionStore.getState().setStatus(nextStatus)
  return nextStatus
}

export function getWsConnectionStatus(): WsConnectionStatus {
  return useWsConnectionStore.getState().status
}

export function useWsConnectionStatus(): WsConnectionStatus {
  return useWsConnectionStore(state => state.status)
}
