...
function addNewTerminalGroup(
  normalized: ThreadTerminalState,
  terminalId: string,
  terminalIds: string[],
  terminalGroups: ThreadTerminalGroup[]
): ThreadTerminalState {
...
function resolveDestinationGroupIndex(
  normalized: ThreadTerminalState,
  terminalGroups: ThreadTerminalGroup[]
): number {
  ...
  return terminalGroups.length - 1
}

function placeTerminalInActiveGroup(
  normalized: ThreadTerminalState,
  terminalId: string,
  terminalIds: string[],
  terminalGroups: ThreadTerminalGroup[],
  ...
  const activeGroupIndex = resolveDestinationGroupIndex(normalized, terminalGroups)
...
function upsertTerminalIntoGroups(
  ...
  if (mode === 'new') {
    return addNewTerminalGroup(normalized, terminalId, terminalIds, terminalGroups)
  }

  return placeTerminalInActiveGroup(
    normalized,
    terminalId,
    terminalIds,
    terminalGroups,
