import { ThreadId, type NativeApi } from '@orxa-code/contracts'
...
function createCheckpointDiffOptions(input: {
  fromTurnCount: number
  toTurnCount: number
  cacheScope: string
}) {
  return checkpointDiffQueryOptions({
    threadId,
    ...input,
  })
}
...
describe('checkpointDiffQueryOptions provider calls', () => {
  it('forwards checkpoint range to the provider API', async () => {
    ...
    const options = createCheckpointDiffOptions({
      fromTurnCount: 3,
      toTurnCount: 4,
      cacheScope: 'turn:abc',
    })
...
describe('checkpointDiffQueryOptions invalid inputs', () => {
...
describe('checkpointDiffQueryOptions retry behavior', () => {
