import type { OrchestrationThreadActivity, ThreadTokenUsageSnapshot } from '@orxa-code/contracts'
...
function deriveContextWindowUsage(payload: Record<string, unknown>) {
...
export function deriveLatestContextWindowSnapshot(
  ...
  for (let index = activities.length - 1; index >= 0; index -= 1) {
    ...
    const usage = deriveContextWindowUsage(payload)
    if (!usage) {
      continue
    }
    return buildContextWindowSnapshot(activity, payload, usage)
  }
