import type { ContextMenuItem } from '@orxa-code/contracts'
  ...
  return new Promise<T | null>(resolve => {
    ...
    for (const item of items) {
      ...
      btn.className = isDisabled
        ? 'flex w-full items-center gap-2 px-3 py-1.5 text-left text-caption text-muted-foreground/60 cursor-not-allowed'
        : isDestructiveAction
          ? 'flex w-full items-center gap-2 px-3 py-1.5 text-left text-caption text-destructive hover:bg-accent cursor-default'
          : 'flex w-full items-center gap-2 px-3 py-1.5 text-left text-caption text-popover-foreground hover:bg-accent cursor-default'
      if (!isDisabled) {
