...
import { InputStartAddon } from '~/components/ui/input.start-addon'
import { type InputAddonProps, resolveInputSize } from '~/components/ui/input.addon-props'
...
function ComboboxInput({
  ...
  size,
  ...props
}: Omit<ComboboxPrimitive.Input.Props, 'size'> & InputAddonProps & { inputClassName?: string }) {
  const sizeValue = resolveInputSize(size)

  return (
    <div className="relative not-has-[>*.w-full]:w-fit w-full text-foreground has-disabled:opacity-64">
      <InputStartAddon slot="combobox-start-addon">{startAddon}</InputStartAddon>
      <ComboboxPrimitive.Input
        className={cn(
          startAddon &&
