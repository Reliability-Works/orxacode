  ...
  return (
    <PlainTextPlugin
      contentEditable={
        <ContentEditable
          className={cn(
            'block max-h-[200px] min-h-17.5 w-full overflow-y-auto whitespace-pre-wrap break-words bg-transparent text-sm leading-relaxed text-foreground focus:outline-none max-sm:text-base',
            props.className
          )}
      ...
      placeholder={
        props.terminalContexts.length > 0 ? null : (
          <div className="pointer-events-none absolute inset-0 text-sm leading-relaxed text-muted-foreground/35 max-sm:text-base">
            {props.placeholder}
          </div>
        )
      }
