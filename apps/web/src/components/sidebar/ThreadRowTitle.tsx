...
export function ThreadStatusLabel({
  ...
  return (
    <span
      title={status.label}
      className={`inline-flex items-center gap-1 text-mini ${status.colorClass}`}
    >
      <span
...
function ChildrenToggle({
  ...
  expanded: boolean
  isMobile: boolean
  onToggle?: ((threadId: ThreadId, expanded: boolean) => void) | undefined
}) {
  return (
    <button
      type="button"
  ...
  return (
    <div className="flex min-w-0 flex-1 items-center gap-2 text-left md:gap-1.5">
      ...
      {rename.isRenaming ? (
        ...
        <span className="min-w-0 flex-1 truncate text-sm text-foreground/75 md:text-xs">
          {thread.title}
        </span>
      )}
    </div>
