    ...
    <div className="flex min-w-0 flex-1 items-center gap-1.5 text-left">
      {hasChildren ? (
        <button
          ...
          className="inline-flex size-4 shrink-0 items-center justify-center rounded-sm text-muted-foreground/70 transition-colors hover:bg-accent hover:text-foreground"
          onClick={event => {
        ...
        >
          <ChevronRightIcon
            className={cn('size-3 transition-transform', childrenExpanded ? 'rotate-90' : '')}
          />
        </button>
      ) : null}
      {provider ? (
