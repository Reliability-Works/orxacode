...
async function startKeybindingsToastSuite() {
...
async function stopKeybindingsToastSuite() {
  await rpcHarness.disconnect()
  await worker.stop()
}

async function resetKeybindingsToastState() {
...
function cleanupKeybindingsToastDom() {
  document.body.innerHTML = ''
}

async function withMountedApp(run: () => Promise<void>) {
...
async function expectConsecutiveKeybindingUpdateToasts() {
  await withMountedApp(async () => {
...
async function expectInvalidKeybindingsToast() {
  await withMountedApp(async () => {
    sendServerConfigUpdatedPush([
...
async function expectNoToastFromReplayedCachedValue() {
  const mounted = await mountApp()
  try {
    sendServerConfigUpdatedPush([])
...
describe('Keybindings update toast', () => {
  beforeAll(startKeybindingsToastSuite)

  afterAll(stopKeybindingsToastSuite)

  beforeEach(resetKeybindingsToastState)

  afterEach(cleanupKeybindingsToastDom)

  it('shows a toast for each consecutive keybinding update with no issues', async () => {
    await expectConsecutiveKeybindingUpdateToasts()
  })

  it('shows a warning toast when keybinding config has issues', async () => {
    await expectInvalidKeybindingsToast()
  })

  it('does not show a toast from the replayed cached value on subscribe', async () => {
    await expectNoToastFromReplayedCachedValue()
  })
})
