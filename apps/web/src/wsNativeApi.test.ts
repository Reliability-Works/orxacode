...
vi.mock('./wsRpcClient', () => {
  return {
    ...
    resetWsRpcClientForTests: vi.fn(),
  }
})
...
function makeDesktopBridge(overrides: Partial<DesktopBridge> = {}): DesktopBridge {
  return {
    getWsUrl: () => null,
    getRemoteAccessSnapshot: async () => ({ enabled: false, port: 0, endpoints: [] }),
...
describe('wsNativeApi server passthrough', () => {
...
describe('wsNativeApi event forwarding', () => {
...
describe('wsNativeApi orchestration and project RPCs', () => {
...
describe('wsNativeApi server update RPCs', () => {
...
describe('wsNativeApi context menu behavior', () => {
