...
export function registerClaudeChatHandlers({ claudeChatService }: ClaudeChatHandlersDeps) {
  ...
  ipcMain.handle(IPC.claudeChatRespondToUserInput, async (_event, requestId: unknown, response: unknown) => {
    return claudeChatService.respondToUserInput(assertString(requestId, "requestId"), assertString(response, "response"));
  });
  ...
  ipcMain.handle(IPC.claudeChatArchiveProviderSession, async (_event, sessionId: unknown, directory?: unknown) => {
    return claudeChatService.archiveProviderSession(assertString(sessionId, "sessionId"), typeof directory === "string" ? directory : undefined);
  });
}
