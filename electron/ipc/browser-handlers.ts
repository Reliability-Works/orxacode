...
function requireBrowserController(getBrowserController: () => BrowserController | null): BrowserController {
...
export function registerBrowserHandlers({
  ...
  ipcMain.handle(IPC.browserSetVisible, async (event, visible: unknown) => {
    assertBrowserSender(event);
    return requireBrowserController(getBrowserController).setVisible(assertBoolean(visible, "visible"));
  });
  ...
  ipcMain.handle(IPC.browserCloseTab, async (event, tabID?: unknown) => {
    assertBrowserSender(event);
    return requireBrowserController(getBrowserController).closeTab(typeof tabID === "string" ? tabID : undefined);
  });
  ...
  ipcMain.handle(IPC.browserBack, async (event, tabID?: unknown) => {
    assertBrowserSender(event);
    return requireBrowserController(getBrowserController).back(typeof tabID === "string" ? tabID : undefined);
  });
  ipcMain.handle(IPC.browserForward, async (event, tabID?: unknown) => {
    assertBrowserSender(event);
    return requireBrowserController(getBrowserController).forward(typeof tabID === "string" ? tabID : undefined);
  });
  ipcMain.handle(IPC.browserReload, async (event, tabID?: unknown) => {
    assertBrowserSender(event);
    return requireBrowserController(getBrowserController).reload(typeof tabID === "string" ? tabID : undefined);
  });
