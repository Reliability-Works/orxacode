...
export function parseOpenCodeAgentFile(filename: string, filePath: string, raw: string): OpenCodeAgentFile {
...
export async function writeOpenCodeAgentFile(filename: string, content: string): Promise<OpenCodeAgentFile> {
...
export async function writeWorkspaceAgentsMd(directory: string, content: string): Promise<AgentsDocument> {
  const root = path.resolve(directory);
  const agentsPath = path.join(root, "AGENTS.md");
