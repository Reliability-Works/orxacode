...
function createHarness(options: HarnessOptions = {}) {
  ...
  const showMessageBox =
    ...
    vi.fn<
      (window: BrowserWindow | null, options: MessageBoxOptions) => Promise<{ response: number }>
    >(async () => ({
      response: 0,
    }))
...
function registerSchedulingTests() {
...
function registerUpdateBehaviorTests() {
  ...
  it('downloads and installs when update is available and user triggers update', async () => {
    ...
    expect(harness.publishTelemetry).toHaveBeenCalledWith(
      expect.objectContaining({ phase: 'update.available', version: '2.0.0' })
    )
    expect(harness.publishTelemetry).toHaveBeenCalledWith(
      expect.objectContaining({ phase: 'download.start', version: '2.0.0' })
    )
    harness.controller.cleanup()
  })
}

function registerProgressAndConcurrencyTests() {
  it('emits completion telemetry and starts install when update download completes', async () => {
    ...
    expect(harness.showMessageBox).not.toHaveBeenCalled()
    expect(harness.publishTelemetry).toHaveBeenCalledWith(
      expect.objectContaining({ phase: 'download.complete', version: '2.0.0' })
    )
    expect(harness.publishTelemetry).toHaveBeenCalledWith(
      expect.objectContaining({ phase: 'install.start', version: '2.0.0' })
    )
  ...
  it('reports progress and handles non-finite progress values', async () => {
    ...
    expect(harness.publishTelemetry).toHaveBeenCalledWith(
      expect.objectContaining({ phase: 'download.progress' })
    )
    harness.controller.cleanup()
  })
...
function registerErrorHandlingTests() {
  ...
  it('handles non-manual updater errors without dialog', async () => {
    ...
    expect(harness.showMessageBox).not.toHaveBeenCalled()
    expect(harness.publishTelemetry).toHaveBeenCalledWith(
      expect.objectContaining({ phase: 'check.error', message: 'Unknown updater error' })
    )
    harness.controller.cleanup()
  })
}
...
describe('createAutoUpdaterController', () => {
  ...
  registerSchedulingTests()
  registerUpdateBehaviorTests()
  registerProgressAndConcurrencyTests()
