...
function createGitWorkspace(prefix: string) {
  const workspaceDir = path.join(tmpdir(), `${prefix}-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
describe("KanbanService", () => {
  ...
  it("creates .worktreeinclude entries from .gitignore", async () => {
    const { service, databasePath } = createService();
    const workspaceDir = path.join(tmpdir(), `orxa-kanban-worktreeinclude-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    try {
      mkdirSync(workspaceDir, { recursive: true });
  ...
  it("restores tracked and untracked changes after trashing a task worktree", async () => {
    ...
    try {
      ...
      expect(readFileSync(path.join(resumed.worktreePath!, "tracked.txt"), "utf8")).toBe("base\nlocal change\n");
