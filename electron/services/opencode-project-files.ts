...
export const PROJECT_FILE_SKIP_DIRS = new Set([".git", "node_modules", ".next", "dist", "build", ".turbo"]);
...
export async function listProjectFiles(directory: string, relativePath = ""): Promise<ProjectFileEntry[]> {
...
export async function readProjectFile(directory: string, relativePath: string): Promise<ProjectFileDocument> {
  const root = path.resolve(directory);
