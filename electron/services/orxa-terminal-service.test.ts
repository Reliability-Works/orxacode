...
function createMockScriptProcess(pid = 201) {
  ...
  return {
    ...
    emitStdout: (chunk: string) =>
      stdoutListeners.forEach(listener => listener(Buffer.from(chunk))),
...
function registerTerminalSpawnFallbackTests() {
  ...
  it('throws a useful error when no shell can be spawned', () => {
    ...
    try {
      const { service } = createTestService()
      const directory = process.cwd()
      expect(() => service.createPty(directory, directory, 'Tab 1')).toThrow(
...
describe('OrxaTerminalService', () => {
  ...
  it('keeps Claude-owned terminals out of workspace terminal listings', () => {
    ...
    spawnNativePtyMock.mockReturnValueOnce(workspacePty).mockReturnValueOnce(claudePty)
  ...
  registerTerminalSpawnFallbackTests()
