...
function createTaskNotificationEvents() {
  return [
    {
      type: 'system',
...
it('maps task and child-thread events into structured background-agent notifications', async () => {
    ...
    const { notifications, states } = captureClaudeChatServiceEvents(service)

    vi.mocked(query).mockReturnValue(createQueryStream(createTaskNotificationEvents()) as never)
    ...
    const thinkingStoppedIndex = notifications.findIndex(
      entry => entry.method === 'thinking/stopped'
    )
  ...
  it('migrates a legacy Claude renderer session id into the provider directory and resumes it', async () => {
    ...
    const setLegacyRendererValue = vi
      .spyOn(directory, 'setLegacyRendererValue')
      ...
      createQueryStream([
        {
          ...
          event: {
            ...
            delta: { partial_json: '{"subagent_type":"Explore"}' },
          },
        },
        {
          type: 'assistant',
          ...
          message: {
            content: [
              {
                type: 'text',
                text: 'Now I can see the website directories. Let me spin up agents.',
              },
              {
                type: 'tool_use',
                id: 'toolu_1',
  ...
  it('renames Claude provider sessions through the SDK', async () => {
    ...
    expect(vi.mocked(renameSession)).toHaveBeenCalledWith('claude-thread-1', 'New Claude Title', {
      dir: '/tmp/project',
    })
  })
  ...
  it('archives a provider child session by tagging it archived', async () => {
    ...
    expect(vi.mocked(tagSession)).toHaveBeenCalledWith('child-session-1', 'archived', {
      dir: '/tmp/project',
    })
  })
  ...
  it('cancels pending Claude approvals and inputs when interrupting a session', async () => {
    ...
    ;(service as unknown as { pendingApprovals: Map<string, unknown> }).pendingApprovals.set(
      'approval-1',
      {
    ...
    ;(service as unknown as { pendingUserInputs: Map<string, unknown> }).pendingUserInputs.set(
      'input-1',
      {
        sessionKey: 'session-interrupt',
        turnId: 'turn-1',
