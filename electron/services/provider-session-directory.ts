...
function parseBinding(raw: string | null): ProviderRuntimeBinding | null {
  ...
  try {
    ...
    if (
      (parsed.provider !== "claude-chat" && parsed.provider !== "codex" && parsed.provider !== "opencode") ||
      typeof parsed.sessionKey !== "string" ||
      (parsed.status !== "starting" && parsed.status !== "running" && parsed.status !== "stopped" && parsed.status !== "error") ||
...
export class ProviderSessionDirectory {
  ...
  getBinding(sessionKey: string, provider?: ProviderRuntimeProvider): ProviderRuntimeBinding | null {
    const normalizedSessionKey = sessionKey.trim();
    if (!normalizedSessionKey) {
      return null;
    }
    const parsed = this.persistenceService
      ? parseBinding(this.persistenceService.getValue(PROVIDER_RUNTIME_NAMESPACE, normalizedSessionKey))
  ...
  upsert(input: ProviderRuntimeBindingInput): ProviderRuntimeBinding {
    ...
    const next: ProviderRuntimeBinding = {
      ...
      resumeCursor: input.resumeCursor !== undefined ? input.resumeCursor : (existing?.resumeCursor ?? null),
      runtimePayload: mergeRuntimePayload(existing?.runtimePayload ?? null, input.runtimePayload),
      updatedAt: new Date().toISOString(),
    };
    if (this.persistenceService) {
      this.persistenceService.setValue(PROVIDER_RUNTIME_NAMESPACE, normalizedSessionKey, JSON.stringify(next));
    } else {
