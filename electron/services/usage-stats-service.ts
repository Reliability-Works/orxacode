...
function estimateCost(model: string, inputTokens: number, cachedInputTokens: number, outputTokens: number): number {
  ...
  for (const filePath of filePaths) {
    ...
    try {
      ...
      for (const line of content.split("\n")) {
        ...
        previousTotals = next.nextTotals;
        ...
        const model = currentModel ?? extractModelFromTokenCount(parsed) ?? "unknown";
...
export async function readCodexUsageStats(): Promise<ProviderUsageStats> {
  ...
  return {
    ...
    modelCount: [...aggregate.modelSessionCounts.keys()].filter((model) => model !== "unknown").length,
