...
export function buildOpenTargetAttempts(input: BuildOpenTargetAttemptsInput): OpenTargetAttempt[] {
  ...
  if (platform === "darwin") {
    ...
    if (target === "antigravity") {
      attempts.push({ command: "open", args: ["-a", "Antigravity", resolvedPath], label: "Antigravity" });
    }
    if (target === "terminal") {
  ...
  if (target === "terminal") {
    if (mode === "directory") {
      attempts.push({ command: "ghostty", args: ["--working-directory", resolvedPath], label: "Ghostty" });
      attempts.push({ command: "x-terminal-emulator", args: ["--working-directory", resolvedPath], label: "Terminal" });
    } else {
