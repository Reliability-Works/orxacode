...
function matchRecord(
  ...
  if (
    query.workspace &&
    normalizeWorkspace(query.workspace) !== normalizeWorkspace(record.workspace)
  ) {
    return false
  }
...
export class ArtifactStore {
  ...
  constructor(options: ArtifactStoreOptions = {}) {
    const base =
      options.rootDir ?? path.join(app.getPath('userData'), ARTIFACTS_DIR, ARTIFACTS_VERSION)
    this.rootDir = base
  ...
  async writeContextSelectionArtifact(
    input: WriteContextTraceArtifactInput
  ...
  async prune(query: { workspace?: string } = {}): Promise<ArtifactPruneResult> {
    ...
    const candidates = fileArtifacts
      .filter(
        item => !normalizedWorkspace || normalizeWorkspace(item.workspace) === normalizedWorkspace
      )
  ...
  async exportBundle(input: ArtifactExportBundleInput): Promise<ArtifactExportBundleResult> {
    ...
    for (const item of selected) {
      ...
      if (item.artifactPath) {
        ...
        const copied = await copyFile(item.artifactPath, destinationPath)
          .then(() => true)
          .catch(() => false)
        if (copied) {
          copiedFiles += 1
