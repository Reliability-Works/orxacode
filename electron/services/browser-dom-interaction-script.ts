...
export type BrowserInteractionMode =
  | 'click'
  | 'type'
  | 'extract_text'
  | 'exists'
...
const INTERACTION_SCRIPT_TEMPLATE = String.raw`(() => {
      const mode = __MODE_JSON__;
      const locator = __LOCATOR_JSON__;
...
export function buildInteractionScript(
  ...
  return INTERACTION_SCRIPT_TEMPLATE
    .replace('__MODE_JSON__', JSON.stringify(mode))
    .replace('__LOCATOR_JSON__', JSON.stringify(locator))
    .replace('__OPTIONS_JSON__', JSON.stringify(options))
}
